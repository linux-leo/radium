...
;; Synchronously replace id-old-instrument with a new instrument created from description.
;; Returns the new instrument ID. Returns old-id unchanged if description is empty.
(define (replace-instrument id-old-instrument description is-current is-current-under-mouse)
...
(delafina (async-replace-instrument :id-old-instrument
  ...
  (define (replace description)
    (replace-instrument id-old-instrument description is-current is-current-under-mouse))

  (cond ((<ra> :instrument-is-permanent id-old-instrument)
         (<ra> :schedule 0
               (lambda ()
                 (ignore-undo-block
                  (lambda ()
         ...
         (<ra> :schedule 0
               (lambda ()
                 (if (<ra> :instrument-is-open-and-audio id-old-instrument)
                     (replace description)
                     (c-display "async-replace-instrument: Note: Instrument " id-old-instrument " is either not open or not audio. It dissapeared"))
                 #f)))))

;; Note: Used for shortcut
(delafina (replace-instrument-popup-menu-entry :instrument-id (<ra> :get-current-instrument-under-mouse)
                                               :must-have-inputs #f
                                               :must-have-outputs #f
                                               :gui -2)
...
;; Called from C++ via S7CALL2(instrument_instrument, "FROM_C-convert-faust-dev1-to-dev2", old-id)
;; Converts a Faust Dev 1 instrument to Faust Dev 2.
(define (FROM_C-convert-faust-dev1-to-dev2 old-id)
  (define is-current (equal? old-id (<ra> :get-current-instrument)))
  (define is-current-under-mouse (equal? old-id (<ra> :get-current-instrument-under-mouse)))
  (define desc (<ra> :get-audio-instrument-description
                      (<ra> :to-base64 "")
                      (<ra> :to-base64 "Faust Dev 2")
                      (<ra> :to-base64 "Faust Dev 2")))

  (replace-instrument old-id desc is-current is-current-under-mouse))

;; Called from C++ via S7CALL2(instrument_instrument, "FROM_C-convert-faust-dev2-to-dev1", old-id)
;; Converts a Faust Dev 2 instrument back to Faust Dev 1.
(define (FROM_C-convert-faust-dev2-to-dev1 old-id)
  (define is-current (equal? old-id (<ra> :get-current-instrument)))
  (define is-current-under-mouse (equal? old-id (<ra> :get-current-instrument-under-mouse)))
  (define desc (<ra> :get-audio-instrument-description
                      (<ra> :to-base64 "")
                      (<ra> :to-base64 "Faust Dev")
                      (<ra> :to-base64 "Faust Dev")))
  ...
  (define plain
    (list
     ...
     (list "Replace..."
           ...
           :shortcut replace-instrument-popup-menu-entry
           (lambda ()
             (replace-instrument-popup-menu-entry instrument-id must-have-inputs must-have-outputs parentgui)))
     (list "Rename..."
